# -*- coding: utf-8 -*-
# ベータ版　更新 2026.1.10 0055 MySpot編集コンプリート版
"""
1. データ統合・取得仕様
...
# ======================================================================================
# 92. 【統合版】操作コントロールパネル（案B改：標準機能で隙間最小化）
# ======================================================================================
def render_compact_control_panel(basho_name):
    """
    CSSを使わず、Streamlitの標準引数(gap="small")と
    ラベルの非表示(collapsed)のみで隙間を詰め、スマホでの縦延びを抑える。
    """
    import streamlit as st
    from datetime import datetime, timedelta

    # 全体を一つのコンテナにまとめ、垂直方向の隙間管理をStreamlitに任せる
    with st.container():
        # --- 1行目：場所 ＋ 星 ---
        display_list, total_data = get_combined_location_list(
            CONFIG["LOCATION_MASTER"], 
        ...
        with c1:
            selected_label = st.selectbox(
                ...
                label_visibility="collapsed" # ラベルの余白をカット
            )
        with c2:
            if is_saved:
                st.button("✅", key="fav_saved_icon", disabled=True, use_container_width=True)
            else:
                if st.button("⭐", key="fav_save_action", use_container_width=True):
        ...
        # --- 2行目：地図 ＋ 現在地 ---
        # ここも gap="small" で左右を詰め、use_container_widthでボタン幅を揃える
        ...
        update_label = f"🔄 グラフ更新 ({now_local.strftime('%m/%d %H:%M')})"
        # type="secondary" で少し控えめな見た目にしつつ、幅はいっぱいに
